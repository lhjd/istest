const plan = (props) => {
  return (
    <div className={`border-2 rounded-lg p-4 flex flex-col items-center w-76 mx-4 ${props.highlight ? 'border-red-600' : 'border-gray-500'}`}>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 63.5 63.5" className={`h-32 w-32 mt-8 fill-current ${props.highlight ? 'text-red-600' : 'text-gray-500'}`}>
      ...
      <h4 className="text-xs text-gray-500 text-center mb-10 pb-2 px-4 border-b border-gray-500">{props.desc2}</h4>
      { props.currentPlan === 'annual' ?
        <h3 className="text-xl font-bold mb-10">{`$${props.pricePerYear}`}/Year</h3>
        :
        < h3 className="text-xl font-bold mb-10">{`$${props.pricePerMonth}`}/Month</h3>
      }
      <button className={` px-4 py-2 rounded mb-8 ${props.highlight ? 'bg-red-600 text-white' : 'bg-white text-gray-500 border border-gray-500'}`}>Get Started</button>
    </div>
  );
}
