const plan = (props) => {
  return (
    <div className={`border-2 rounded-lg p-2 xl:p-4 flex flex-col items-center w-76 mb-4 mx-2 xl:mx-4 ${props.highlight ? 'border-red-600' : 'border-gray-500'}`}>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 63.5 63.5" className={`hidden xl:block h-32 w-32 mt-8 fill-current ${props.highlight ? 'text-red-600' : 'text-gray-500'}`}>
      ...
      <button className={` px-4 py-2 rounded mb-4 xl:mb-8 ${props.highlight ? 'bg-red-600 text-white' : 'bg-white text-gray-500 border border-gray-500'}`}>Get Started</button>
    </div>
  );
}
